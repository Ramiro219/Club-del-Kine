...
export function TratamientoDetailPage() {
  ...
  const load = useCallback(async () => { try { setRow(await getTratamiento(id)) } catch (error) { toast.error(error instanceof Error ? error.message : 'No se pudo cargar el tratamiento.') } }, [id])
